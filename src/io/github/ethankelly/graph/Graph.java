...
public class Graph implements Cloneable {
    ...
    /**
     * Class constructor. This assigns an empty list within the adjacency list for as many vertices as required
     *
    ...
    public Graph(int numVertices, String name) {
        // Assign given number of vertices and the name of the graph
        this.numVertices = numVertices;
        this.name = name;
        // Add an empty list for each vertex in the graph in the adjacency list
        IntStream.range(0, numVertices).forEach(i -> adjList.add(i, new ArrayList<>()));
        // Initialise discovery time (for DFS cut-vertex detection) to 0
        this.time = 0;
    }
    ...
    private static void spliceUtil(List<Graph> subGraphs, Vertex cutVertex, Graph clone, Graph original) {
        ...
        // Iterate over connected components after cut vertex removal
        for (List<Vertex> cc : clone.getCCs()) {
            ...
            if (cc.size() > 1) {
                ...
                // If the graph cannot be spliced further, add it to the list to return, else recurse
    ...
    public List<Graph> splice() {
        ...
        // If there are cut vertices, splice based on them, otherwise return the current graph (cannot be spliced)
    ...
    public List<Vertex> getCutVertices() {
        ...
        // Initialize parent and visited and cut-vertex arrays
    ...
    public void removeVertex(Vertex v) {
        ...
        // Iterate through all vertices in each list and if the current vertex is v, remove it
        for (List<Vertex> list : adjList) list.removeIf(w -> w.equals(v));
    ...
    /**
     * Overloaded method - returns true if an edge exists between the vertices at the given integer locations, false
     * otherwise.
     *
     * @param i start integer location of the edge to be queried in the graph.
     * @param j end integer location of the edge to be queried in the graph.
     * @return true if the vertices at i and j share an edge, false otherwise.
     */
    ...
    /**
     * Used to create an identical instance of the current graph object by copying each of the original attributes to
     * a new Graph object (known as a deep copy).
     *
    ...
    public boolean equals(Object o) {
        ...
        Graph that = (Graph) o;
        // Check adjacency lists are equal
    ...
    /**
     * Recalculates the edges in a graph by traversing the adjacency lists and counting occurrences and assigns the
     * newly calculated value to the current graph.
     *
     * @return the number of edges in the current graph.
     */
    public int recalculateEdges() {
        int sum = 0;
        // traverse all vertices and add all edges linked to the current vertex
        for (int i = 0; i < this.getNumVertices(); i++) sum += this.getAdjList().get(i).size();
        int newEdges = sum / 2;
        this.setNumEdges(newEdges);
        return newEdges;
    }

    /**
     * @return unique integer values for identical Graph instances, distinct integers for different instances.
     */
    @Override
    public int hashCode() {
