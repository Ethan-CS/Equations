...
public class Vertex implements Comparable<Vertex> {
    ...
    public static boolean areStatesDifferent(List<Vertex> toCheck) {
        ...
        if (toCheck.size() == 1) {
            ...
            for (Vertex v : toCheck) {
                if (toCheck.stream().anyMatch(w -> v.getState() != w.getState())) {
                    statesDifferent = true;
                }
            }
        }
     ...
     *
     * @param toCheck the tuple we wish to verify has no repeated vertex locations.
    ...
    public static boolean areLocationsDifferent(List<Vertex> toCheck) {
        ...
        // We only need to find two vertices in the list that have the same index location
    ...
    /**
     * Given some list of vertices (some tuple), this method verifies that all of the vertices in the graph given are in
     * fact connected. That is, we are only interested in a tuple of vertices that constitute some manner of path (cycle
     * or such like) - if not, then we do not have to consider the associated equation in the final system of equations
    ...
    public static boolean areAllConnected(List<Vertex> toCheck, Graph g) {
        ...
        if (toCheck.size() == 1) {
            ...
            // Return whether the vertices have some path between them
